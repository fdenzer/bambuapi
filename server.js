...
app.get('/api/printer-status', async (req, res) => {
    ...
    try {
        ...
        // Add information about available printers
        responseData.available_printers = data.devices.map(device => {
            const config = configuredPrinters.find(printer =>
                (typeof printer === 'string' && printer === device.dev_id) ||
                (typeof printer === 'object' && printer.serial === device.dev_id)
            );
            return {
...
// Start the server
    app.listen(port, '0.0.0.0', () => {
        ...
        console.log('Der Access Token wird in der Session gespeichert.')
    })
})